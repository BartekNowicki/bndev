...
interface BaloonState {
  myTween: gsap.core.Tween | null;
  deltaX: number;
  deltaY: number;
  color: string,
  myElement: any; 
...
class Baloon extends Component<BaloonProps, BaloonState> {
  ...
  constructor(props: BaloonProps) {
    super(props);
    this.deltaX = 0;
    this.deltaY = 0;
    this.color = this.props.color;
    this.myElement = null;
    this.colors = ['red', 'green', 'blue', 'darkblue', 'black', 'brown', 'coral', 'yellow','white','pink', 'orange'];
  }
  ...
  componentDidMount() {
      ...
      //tween for initiation when DOM mounted
  ...
  flickerBaloonMain = (element: gsap.TweenTarget) => {
...
flickerBaloonReflection = (element: gsap.TweenTarget) => {
...
waveBaloonRope = (element: gsap.TweenTarget) => {
...
stretchBaloonRope = (element: gsap.TweenTarget) => {
  ...
  setNewBaloonPosition = (event: { clientX: number; clientY: number; }) => {
      const { clientX, clientY } = event;
      const { ratioX, ratioY } = this.props;
      const screenCenterX = window.innerWidth / 2;
      const screenCenterY = window.innerHeight / 2;
      const deltaX: number = clientX - screenCenterX;
      const deltaY: number = clientY - screenCenterY;
      this.setState({
      ...
      <g id="g3237">
        <g id="g3965" transform="matrix(.72886 0 0 .72886 594.67 100.02)">
          <g id="g3797" transform="translate(-75.901 -162.06)">          
            <path id="baloonMain" fill = {this.color}
  ...
  render() {
    ...
    return (
	<>
    <div ref={div => this.myElement = div} className = {className} style = {{transform: `translate(${this.deltaX}px, ${this.deltaY}px)`}}>
    {this.drawTheBaloon()}     
    </div>
    </>
	);
